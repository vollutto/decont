...
do
	...
	then
		echo "Already merged the samples of $sid into a single file"
	else
		echo "Running Merge..."
...
do
	if [ -f "out/trimmed/${sampleid}.trimmed.fastq.gz" ]
	then
		echo "Already run the cutadapt for $sampleid merged files"
	else
...
do
	...
	then
		echo "Already run STAR for $sid trimmed files"
	else
		echo "Running STAR..."
